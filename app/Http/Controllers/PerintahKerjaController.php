...
class PerintahKerjaController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $lampiran=LampiranOrder::orderBy('nomor_order','desc')->whereIn('status',['1','3'])->get();
            //  dd($lampiran);
            return datatables()->of($lampiran)
                    ->addColumn('action', 'action_butt_perintah')
                    ->rawColumns(['action'])
                    ->addIndexColumn()
                    ->make(true);
        }
    ...
    public function indexLapor()
    {
        ...
        if (request()->ajax()) {
            ...
            $lampiran=LampiranOrder::orderBy('nomor_order','desc')->whereNotIn('status',['0'])->get();
            //  dd($lampiran);
            return datatables()->of($lampiran)
                    ->addColumn('action', 'action_butt_perintah')
                    ->rawColumns(['action'])
...
// dd($request->id);
        LampiranOrder::whereId($request->id)->update($form_data);
        return response()->json(['success' => 'Update Status Berhasil Disimpan']);
    }
